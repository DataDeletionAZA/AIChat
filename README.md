...
- TTS 闲置自动休眠
  - 默认开启。语音服务连续 5 分钟未被使用时，Mod 会关闭本机的 GPT-SoVITS 来释放内存。
  - 下一次需要合成语音或进行语音识别时，Mod 会通过 `TTS 服务脚本文件路径` 自动重新启动服务。
  - 可在游戏内的 `TTS 配置` 中关闭此功能，或把等待时间调整为 1 至 30 分钟。
  - 自动休眠只会操作 `127.0.0.1`、`localhost` 或 `::1` 上的本机服务，不会关闭远程 TTS 服务。
